 ...
 *   16px desk slot picks it up without passing anything.
 *
...
const letters: Record<string, string> = {
  ...
  txt: 'T',
  json: '{}',
  // Web files stay lettered: no vendor glyph reads well at 20px.
  html: '<>',
  htm: '<>',
  css: '#',
  js: 'JS',
...
export function FileBadge({ name, size = 'sm', bare = false }: { name: string; size?: BadgeSize; bare?: boolean }) {
  ...
  // aria-hidden: the accessible name lives on the row/tab that holds the badge.
  const icon = (
